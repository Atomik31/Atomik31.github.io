# CV GitHub

Portfolio de **Julien Charlier** — Architecte Data / IA · IoT (LoRaWAN) & Modélisation prédictive · Double compétence OT-IT.

> Je relie le terrain industriel à l'intelligence artificielle, de la donnée capteur terrain au déploiement en production de modèles prédictifs.

🔗 **Site en ligne : [atomik31.github.io](https://atomik31.github.io/)**

## Aperçu

- **Parcours** : plus de 14 ans de terrain industriel — ascenseurs inclinés, automatisme, domotique, IoT, support technique expert Toyota — puis reconversion en Architecte Data / IA (certification RNCP niveau 7, Jedha)
- **Compétences** : Python, machine learning & deep learning, pipelines MLOps (Airflow, MLflow, Docker, Kubernetes), cloud AWS, IoT & embarqué (ESP32, LoRaWAN), computer vision (YOLO, OpenCV)
- **Projets** : maintenance prédictive d'éoliennes (pipeline MLOps de bout en bout), détection de fraude bancaire en temps réel, détection d'objets sur flux RTSP, dashboard temps réel des parkings d'Aix-en-Provence
- **Langues** : Français / Anglais
- Site statique en HTML / CSS / JavaScript vanilla, sans framework ni dépendance

## Structure

```
├── index.html            # Point d'entrée
├── styles.css            # Styles
├── app.js                # Rendu du contenu et bascule FR/EN
├── data/portfolio.json   # Tout le contenu du site (bilingue)
└── assets/               # Images et CV
```

Tout le contenu est centralisé dans `data/portfolio.json` : modifier ce fichier suffit à mettre le site à jour.

## Contact

- GitHub : [@Atomik31](https://github.com/Atomik31)
- LinkedIn : [Julien Charlier](https://www.linkedin.com/in/julien-charlier-3a41b779/)
